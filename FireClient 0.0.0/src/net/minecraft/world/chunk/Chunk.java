...
public class Chunk {
	...
	public void generateHeightMap() {
		...
		for(int j = 0; j < 16; j++) {
			for(int k = 0; k < 16; k++) {
				...
				int l = worldObj.worldYMax - 1;
				for(int i1 = j << worldObj.chunkXZCoordinateShift | k << worldObj.worldYBits; l > 0 && Block.lightOpacity[blocks[(i1 + l) - 1] & 0xff] == 0; l--) {
				}
	...
	public void generateSkylightMap() {
		...
		for(int j = 0; j < 16; j++) {
			for(int l = 0; l < 16; l++) {
				...
				int k1;
				for(k1 = j << worldObj.chunkXZCoordinateShift | l << worldObj.worldYBits; j1 > 0 && Block.lightOpacity[blocks[(k1 + j1) - 1] & 0xff] == 0; j1--) {
				}
	...
	private void relightBlock(int i, int j, int k) {
		...
		for(int j1 = i << worldObj.chunkXZCoordinateShift | k << worldObj.worldYBits; i1 > 0 && Block.lightOpacity[blocks[(j1 + i1) - 1] & 0xff] == 0; i1--) {
		}
		if(i1 == l) {
			return;
		}
	...
	public int getBlockID(int i, int j, int k) {
		return blocks[i << worldObj.chunkXZCoordinateShift | k << worldObj.worldYBits | j] & 0xff;
	}
	...
	public boolean setBlockIDWithMetadata(int i, int j, int k, int l, int i1) {
		...
		int l1 = blocks[i << worldObj.chunkXZCoordinateShift | k << worldObj.worldYBits | j] & 0xff;
		if(l1 == l && data.getNibble(i, j, k) == i1) {
			return false;
		}
		int i2 = xPosition * 16 + i;
		int j2 = zPosition * 16 + k;
		blocks[i << worldObj.chunkXZCoordinateShift | k << worldObj.worldYBits | j] = (byte) (byte0 & 0xff);
		if(l1 != 0) {
	...
	public boolean setBlockID(int i, int j, int k, int l) {
		...
		int k1 = blocks[i << worldObj.chunkXZCoordinateShift | k << worldObj.worldYBits | j] & 0xff;
		if(k1 == l) {
			return false;
		}
		int l1 = xPosition * 16 + i;
	...
	public int setChunkData(byte abyte0[], int i, int j, int k, int l, int i1, int j1, int k1) {
		for(int l1 = i; l1 < l; l1++) {
			for(int l2 = k; l2 < j1; l2++) {
				int l3 = l1 << worldObj.chunkXZCoordinateShift | l2 << worldObj.worldYBits | j;
				int l4 = i1 - j;
		...
		for(int i2 = i; i2 < l; i2++) {
			for(int i3 = k; i3 < j1; i3++) {
				int i4 = (i2 << worldObj.chunkXZCoordinateShift | i3 << worldObj.worldYBits | j) >> 1;
				int i5 = (i1 - j) / 2;
		...
		for(int j2 = i; j2 < l; j2++) {
			for(int j3 = k; j3 < j1; j3++) {
				int j4 = (j2 << worldObj.chunkXZCoordinateShift | j3 << worldObj.worldYBits | j) >> 1;
				int j5 = (i1 - j) / 2;
		...
		for(int k2 = i; k2 < l; k2++) {
			for(int k3 = k; k3 < j1; k3++) {
				int k4 = (k2 << worldObj.chunkXZCoordinateShift | k3 << worldObj.worldYBits | j) >> 1;
				int k5 = (i1 - j) / 2;
