...
public class BlockCauldron extends Block
{
    ...
    public boolean blockActivated(World world, int i, int j, int k, EntityPlayer entityplayer)
    {
        ...
        if(itemstack.itemID == Item.WATER_BUCKET.id)
        {
            if(l < 3)
            {
                if(!entityplayer.abilities.depleteBuckets)
                {
                    entityplayer.inventory.setInventorySlotContents(entityplayer.inventory.currentItem, new ItemStack(Item.EMPTY_BUCKET));
                }
                world.setBlockMetadataWithNotify(i, j, k, 3);
            }
            return true;
        }
        if(itemstack.itemID == Item.GLASS_BOTTLE.id && l > 0)
        {
