...
import net.minecraft.entity.TileEntityEnchantmentTable;
		...
		for(int l = i - 2; l <= i + 2; l++) {
			for(int i1 = k - 2; i1 <= k + 2; i1++) {
				...
				for(int j1 = j; j1 <= j + 1; j1++) {
					...
					world.spawnParticle("enchantmenttable", (double) i + 0.5D, (double) j + 2D, (double) k + 0.5D, (double) ((float) (l - i) + random.nextFloat()) - 0.5D, (float) (j1 - j) - random.nextFloat() - 1.0F,
							(double) ((float) (i1 - k) + random.nextFloat()) - 0.5D);
				}
