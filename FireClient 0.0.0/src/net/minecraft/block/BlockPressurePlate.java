...
public class BlockPressurePlate extends Block
{
    ...
    private void setStateIfMobInteractsWithPlate(World world, int i, int j, int k)
    {
        ...
        {
            list = world.getEntitiesWithinAABB(net.minecraft.entity.EntityLiving.class, AxisAlignedBB.getBoundingBoxFromPool((float)i + f, j, (float)k + f, (float)(i + 1) - f, (double)j + 0.25D, (float)(k + 1) - f));
        }
        if(triggerMobType == EnumMobType.players)
        {
            list = world.getEntitiesWithinAABB(net.minecraft.entity.EntityPlayer.class, AxisAlignedBB.getBoundingBoxFromPool((float)i + f, j, (float)k + f, (float)(i + 1) - f, (double)j + 0.25D, (float)(k + 1) - f));
        }
        if(list.size() > 0)
        {
            flag1 = true;
