...
public abstract class Entity {
	...
	public void onEntityUpdate() {
		...
		if(isSprinting()) {
			...
			if(j1 > 0) {
				worldObj.spawnParticle((new StringBuilder()).append("tilecrack_").append(j1).toString(), posX + ((double) rand.nextFloat() - 0.5D) * (double) width, boundingBox.minY + 0.10000000000000001D,
						posZ + ((double) rand.nextFloat() - 0.5D) * (double) width, -motionX * 4D, 1.5D, -motionZ * 4D);
			}
		}
	...
	public void setUsingElytra(boolean flag) {
		setFlag(6, flag);
	}
