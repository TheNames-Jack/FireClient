...
import net.minecraft.util.ActiveRenderInfo;
import net.minecraft.util.EffectRenderer;
import net.minecraft.util.Frustrum;
...
import net.minecraft.util.math.ClippingHelperImpl;
...
public class EntityRenderer {
		...
		if(entityliving.isPlayerSleeping()) {
			...
			if(mc.gameSettings.debugCamEnable) {
				...
				for(int l = 0; l < 8; l++) {
					...
					f7 *= 0.1F;
					f8 *= 0.1F;
					MovingObjectPosition movingobjectposition = mc.theWorld.rayTraceBlocks(Vec3D.createVector(d + (double) f6, d1 + (double) f7, d2 + (double) f8),
							Vec3D.createVector((d - d4) + (double) f6 + (double) f8, (d1 - d6) + (double) f7, (d2 - d5) + (double) f8));
					if(movingobjectposition == null) {
						continue;
					}
	...
	private void renderHand(float f, int i) {
		...
		if(mc.gameSettings.thirdPersonView == 0 && !mc.renderViewEntity.isPlayerSleeping() && !mc.gameSettings.hideGUI && !mc.playerController.displayHUD()) {
			enableLightmap(f);
	...
	private void addRainParticles() {
		...
		if(mc.gameSettings.field_41087_P == 1) {
			i1 >>= 1;
		}else if(mc.gameSettings.field_41087_P == 2) {
			i1 = 0;
		}
