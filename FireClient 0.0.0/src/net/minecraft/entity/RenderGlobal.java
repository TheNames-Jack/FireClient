...
import net.minecraft.item.ItemDisc;
import net.minecraft.item.ItemStack;
import net.minecraft.util.EnumMovingObjectType;
import net.minecraft.util.GLAllocation;
import net.minecraft.util.ImageBufferDownload;
...
public class RenderGlobal implements IWorldAccess {
	...
	public void loadRenderers() {
		...
		renderChunksTall = worldObj.worldYMax / 16;
	...
	public void renderEntities(Vec3D vec3d, ICamera icamera, float f) {
		...
		for(int j = 0; j < list.size(); j++) {
			...
			if(!entity1.isInRangeToRenderVec3D(vec3d) || !entity1.ignoreFrustumCheck && !icamera.isBoundingBoxInFrustum(entity1.boundingBox)
					|| entity1 == mc.renderViewEntity && mc.gameSettings.thirdPersonView == 0 && !mc.renderViewEntity.isPlayerSleeping()) {
				continue;
			}
			...
			if(l >= worldObj.worldYMax) {
				l = worldObj.worldYMax - 1;
			}
	...
	public String getDebugInfoRenders() {
		return (new StringBuilder()).append("C: ").append(renderersBeingRendered).append("/").append(renderersLoaded).append(". F: ").append(renderersBeingClipped).append(", O: ").append(renderersBeingOccluded).append(", E: ")
				.append(renderersSkippingRenderPass).toString();
	}

	public String getDebugInfoEntities() {
		return (new StringBuilder()).append("E: ").append(countEntitiesRendered).append("/").append(countEntitiesTotal).append(". B: ").append(countEntitiesHidden).append(", I: ").append(countEntitiesTotal - countEntitiesHidden - countEntitiesRendered)
				.toString();
	}
	...
	public EntityFX func_40193_b(String s, double d, double d1, double d2, double d3, double d4, double d5) {
	...
	public void obtainEntitySkin(Entity entity) {
		...
		if(entity.capeURL != null) {
			renderEngine.obtainImageData(entity.capeURL, new ImageBufferDownload());
		}
	}
	...
	public void releaseEntitySkin(Entity entity) {
		...
		if(entity.capeURL != null) {
			renderEngine.releaseImageData(entity.capeURL);
		}
	}
	...
	public void playAuxSFX(EntityPlayer entityplayer, int i, int j, int k, int l, int i1) {
		...
		switch(i) {
			...
			case 2003:
				...
				String s = (new StringBuilder()).append("iconcrack_").append(Item.ENDER_EYE.id).toString();
			...
			case 2002:
				...
				double d5 = k;
				double d9 = l;
				String s1 = (new StringBuilder()).append("iconcrack_").append(Item.POTION.id).toString();
			...
			case 1005:
				if(Item.itemsList[i1] instanceof ItemDisc) {
					worldObj.playRecord(((ItemDisc) Item.itemsList[i1]).recordName, j, k, l);
				}else {
