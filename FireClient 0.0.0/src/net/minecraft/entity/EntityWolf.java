...
public class EntityWolf extends EntityAnimal
{
    ...
    protected void updateEntityActionState()
    {
        ...
        {
            List list = worldObj.getEntitiesWithinAABB(net.minecraft.entity.EntitySheep.class, AxisAlignedBB.getBoundingBoxFromPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(16D, 4D, 16D));
            if(!list.isEmpty())
            {
    ...
    public void onLivingUpdate()
    {
        ...
        if(hasCurrentTarget() && !hasPath() && !isWolfAngry())
        {
            ...
            if(entity instanceof EntityPlayer)
            {
                ...
                if(itemstack != null)
                {
                    if(!isWolfTamed() && itemstack.itemID == Item.BONE.id)
                    {
    ...
    public boolean interact(EntityPlayer entityplayer)
    {
        ...
        if(!isWolfTamed())
        {
            if(itemstack != null && itemstack.itemID == Item.BONE.id && !isWolfAngry())
            {
                itemstack.stackSize--;
