...
import net.minecraft.util.EnumStatus;
import net.minecraft.util.FoodStats;
import net.minecraft.util.MathHelper;
import net.minecraft.util.PlayerInfo;
...
public abstract class EntityPlayer extends EntityLiving {
	...
	public EntityPlayer(World world) {
		...
		timeUntilPortal = 20;
		inPortal = false;
		abilities = new PlayerCapabilities();
		speedOnGround = 0.1F;
		speedInAir = 0.02F;
		fishEntity = null;
	...
	public void onUpdate() {
		...
		if(isBurning() && abilities.disableDamage) {
			func_40045_B();
		}
	...
	public void updateCloak() {
		playerCapeURL = PlayerInfo.GetCapeURL(playerUUID);
	...
	public void onDeath(DamageSource damagesource) {
		...
		if(username.equals("Notch")) {
			dropPlayerItemWithRandomChoice(new ItemStack(Item.APPLE, 1), true);
		}
	...
	public void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		abilities.func_40600_b(nbttagcompound);
	}
	...
	public void writeEntityToNBT(NBTTagCompound nbttagcompound) {
		...
		abilities.func_40601_a(nbttagcompound);
	}
	...
	public boolean attackEntityFrom(DamageSource damagesource, int i) {
		if(abilities.disableDamage && !damagesource.canHarmInCreative()) {
			return false;
		}
		entityAge = 0;
		if(getEntityHealth() <= 0) {
	...
	protected void alertWolves(EntityLiving entityliving, boolean flag) {
		...
		List list = worldObj.getEntitiesWithinAABB(net.minecraft.entity.EntityWolf.class, AxisAlignedBB.getBoundingBoxFromPool(posX, posY, posZ, posX + 1.0D, posY + 1.0D, posZ + 1.0D).expand(16D, 4D, 16D));
	...
	public EnumStatus sleepInBedAt(int i, int j, int k) {
		if(!worldObj.multiplayerWorld) {
			...
			double d = 8D;
			double d1 = 5D;
			List list = worldObj.getEntitiesWithinAABB(net.minecraft.entity.EntityMob.class, AxisAlignedBB.getBoundingBoxFromPool((double) i - d, (double) j - d1, (double) k - d, (double) i + d, (double) j + d1, (double) k + d));
			if(!list.isEmpty()) {
	...
	public void moveEntityWithHeading(float strafe, float forward) {
		...
		if(abilities.isFlying) {
			...
			jumpMovementFactor = abilities.GetFlySpeed();
			super.moveEntityWithHeading(strafe, forward);
			motionY = prevMotionY * 0.59999999999999998D; //Dampen vertical motion when flying
			jumpMovementFactor = originalJumpFactor;
			canUseElytra = false;
			setUsingElytra(false);
		}else {
			super.moveEntityWithHeading(strafe, forward);
		}
		addMovementStat(posX - prevX, posY - prevY, posZ - prevZ);
	}
	...
	public int getItemIcon(ItemStack itemstack, int i) {
		...
		if(itemstack.itemID == Item.FISHING_ROD.id && fishEntity != null) {
			j = itemstack.getIconIndex() + 16;
		}else {
			if(itemstack.itemID == Item.POTION.id) {
				if(i == 1) {
			...
			if(itemInUse != null && itemstack.itemID == Item.BOW.id) {
	...
	public boolean func_35197_b(boolean flag) {
		return (flag || foodStats.needFood()) && !abilities.disableDamage;
	}
