...
public abstract class EntityLiving extends Entity {
	...
	protected void onDeathUpdate() {
		...
		if(deathTime == 20) {
			if(!worldObj.multiplayerWorld && (field_34905_c > 0 || func_35163_av()) && !func_40127_l()) {
				for(int i = func_36001_a(player); i > 0;) {
	...
	public void spawnExplosionParticle() {
		for(int i = 0; i < 20; i++) {
			...
			double d2 = rand.nextGaussian() * 0.02D;
			double d3 = 10D;
			worldObj.spawnParticle("explode", (posX + (double) (rand.nextFloat() * width * 2.0F)) - (double) width - d * d3, (posY + (double) (rand.nextFloat() * height)) - d1 * d3,
	...
	public void moveEntityWithHeading(float strafe, float forward) {
		if(canUseElytra && isUsingElytra() && !onGround && !inWater && !isPlayerSleeping()) {
			if(motionY > -0.5D) {
				fallDistance = 1.0F;
			}
			Vec3D lookVector = getLookVec();
			float pitchRadians = rotationPitch * 0.017453292F;
			double horizontalSpeed = Math.sqrt(lookVector.xCoord * lookVector.xCoord + lookVector.zCoord * lookVector.zCoord);
			double currentSpeed = Math.sqrt(motionX * motionX + motionZ * motionZ);
			// Apply lift and drag
			double lookLength = lookVector.lengthVector();
			float pitchFactor = MathHelper.cos(pitchRadians);
			pitchFactor = (float) (pitchFactor * pitchFactor * Math.min(1.0D, lookLength / 0.4D));
			motionY += -0.08D + pitchFactor * 0.06D;
			if(motionY < 0.0D && horizontalSpeed > 0.0D) {
				double adjustment = motionY * -0.1D * pitchFactor;
				motionY += adjustment;
				motionX += lookVector.xCoord * adjustment / horizontalSpeed;
				motionZ += lookVector.zCoord * adjustment / horizontalSpeed;
			}
			if(pitchRadians < 0.0F) {
				double diveBoost = currentSpeed * (-MathHelper.sin(pitchRadians)) * 0.04D;
				motionY += diveBoost * 3.2D;
				motionX -= lookVector.xCoord * diveBoost / horizontalSpeed;
				motionZ -= lookVector.zCoord * diveBoost / horizontalSpeed;
			}
			if(horizontalSpeed > 0.0D) {
				motionX += (lookVector.xCoord / horizontalSpeed * currentSpeed - motionX) * 0.1D;
				motionZ += (lookVector.zCoord / horizontalSpeed * currentSpeed - motionZ) * 0.1D;
			}
			// Apply drag
			motionX *= 0.99D;
			motionY *= 0.98D;
			...
			if(isCollidedHorizontally && !worldObj.multiplayerWorld) {
				...
				if(damage > 0.0F) {
					worldObj.playSoundAtEntity(this, getHurtSound(), 1F, 1F);
					attackEntityFrom(DamageSource.flyIntoWall, (int) damage);
				}
			}
		}else {
			setUsingElytra(false);
			if(isInWater()) {
				double d = posY;
