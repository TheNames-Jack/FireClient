...
public class EntityMooshroom extends EntityCow
{
    ...
    public boolean interact(EntityPlayer entityplayer)
    {
        ...
        if(itemstack != null && itemstack.itemID == Item.EMPTY_BOWl.id && func_40146_g() >= 0)
        {
            entityplayer.inventory.setInventorySlotContents(entityplayer.inventory.currentItem, new ItemStack(Item.SOUP_BOWL));
            return true;
        }
        if(itemstack != null && itemstack.itemID == Item.SHEARS.id && func_40146_g() >= 0)
        {
            setEntityDead();
