...
import net.minecraft.util.ChatLine;
...
public class GuiIngame extends Gui {
	...
	public void renderGameOverlay(float f, boolean flag, int i, int j) {
		...
		if(!mc.playerController.func_35643_e()) {
			...
			int playerHealth = mc.thePlayer.getEntityHealth();
			int previousPlayerHealth = mc.thePlayer.prevHealth;
			...
			if(mc.thePlayer.playerLevel > 0 && mc.playerController.isInCreativeMode()) {
				...
				if(xpBarCap > 0) {
					...
					int l9 = (l - 32) + 3;
					drawTexturedModalRect(xpBarX, l9, 0, 64, c, 5);
					if(i9 > 0) {
						drawTexturedModalRect(xpBarX, l9, 0, 69, i9, 5);
					}
				}
			}
			...
			if(mc.playerController.shouldDrawHUD()) {
				...
				for(int l10 = 0; l10 < 10; l10++) {
					...
					if(flag2) {
						if(l10 * 2 + 1 < previousPlayerHealth) {
				...
				for(int i11 = 0; i11 < 10; i11++) {
					...
					if(mc.thePlayer.getFoodStats().getFoodSaturationLevel() <= 0.0F && updateCounter % (foodLevel * 3 + 1) == 0) {
		...
		if(mc.gameSettings.showDebugInfoMore) {
			GL11.glPushMatrix();
			if(Minecraft.hasPaidCheckTime > 0L) {
				GL11.glTranslatef(0.0F, 32F, 0.0F);
			}
			fontrenderer.drawStringWithShadow((new StringBuilder()).append(IINFO.name + ": " + IINFO.version + " (").append(mc.debug).append(")").toString(), 2, 2, 0xffffff);
			...
			GL11.glPopMatrix();
		}else if(mc.gameSettings.showDebugInfoLess) {
			GL11.glPushMatrix();
			if(Minecraft.hasPaidCheckTime > 0L) {
				GL11.glTranslatef(0.0F, 32F, 0.0F);
			}
			String fps = "Fps: " + mc.realPlayerFPS;
			fontrenderer.drawStringWithShadow(fps, 2, 2, 0xFFFFFF);
			fontrenderer.drawStringWithShadow((new StringBuilder()).append("X: ").append((int)mc.thePlayer.posX).toString(), 2, 16, 0xFFFFFF);
			fontrenderer.drawStringWithShadow((new StringBuilder()).append("Y: ").append((int)mc.thePlayer.posY).toString(), 2, 24, 0xFFFFFF);
			fontrenderer.drawStringWithShadow((new StringBuilder()).append("Z: ").append((int)mc.thePlayer.posZ).toString(), 2, 32, 0xFFFFFF);
			GL11.glPopMatrix();
		}
		...
		if((mc.thePlayer instanceof EntityClientPlayerMP) && mc.gameSettings.keyBindPlayerList.pressed) {
			...
			for(int j8 = 0; j8 < i4; j8++) {
				...
				fontrenderer.drawStringWithShadow(guisavinglevelstring.username, l8, k9, 0xffffff);
				mc.renderEngine.bindTexture(mc.renderEngine.getTexture("/gui/icons.png"));
				int k10 = 0;
				byte byte3 = 0;
				k10 = 0;
				byte3 = 0;
				if(guisavinglevelstring.ping < 0) {
					byte3 = 5;
				}else if(guisavinglevelstring.ping < 150) {
					byte3 = 0;
				}else if(guisavinglevelstring.ping < 300) {
					byte3 = 1;
				}else if(guisavinglevelstring.ping < 600) {
					byte3 = 2;
				}else if(guisavinglevelstring.ping < 1000) {
					byte3 = 3;
				}else {
					byte3 = 4;
				}
