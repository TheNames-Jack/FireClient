...
public class FontRenderer {
	...
	public FontRenderer(GameSettings gamesettings, String s, RenderEngine renderengine) {
		...
		try {
			bufferedimage = ImageIO.read((net.minecraft.entity.RenderEngine.class).getResourceAsStream(s));
	...
	public void func_40609_a(String s, int i, int j, int k, int l, boolean flag) {
		...
		do {
			...
			String s2;
			for(s2 = (new StringBuilder()).append(s1).append(as1[j1++]).append(" ").toString(); j1 < as1.length
					&& getStringWidth((new StringBuilder()).append(s2).append(as1[j1]).toString()) < k; s2 = (new StringBuilder()).append(s2).append(as1[j1++]).append(" ").toString()) {
			}
			int k1;
	...
	public int splitStringWidth(String s, int i) {
		...
		do {
			...
			String s1;
			for(s1 = (new StringBuilder()).append(as1[l++]).append(" ").toString(); l < as1.length
					&& getStringWidth((new StringBuilder()).append(s1).append(as1[l]).toString()) < i; s1 = (new StringBuilder()).append(s1).append(as1[l++]).append(" ").toString()) {
			}
			int j1;
