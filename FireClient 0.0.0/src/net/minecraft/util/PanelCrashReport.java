...
public class PanelCrashReport extends Panel {
	...
	public PanelCrashReport(UnexpectedThrowable unexpectedthrowable) {
		...
		try {
			...
			s2 = (new StringBuilder()).append(s2).append("OpenGL: ").append(GL11.glGetString(7937 /* GL_RENDERER */)).append(" version ").append(GL11.glGetString(7938 /* GL_VERSION */)).append(", ").append(GL11.glGetString(7936 /* GL_VENDOR */))
					.append("\n").toString();
		...
		if(s.contains("Pixel format not accelerated")) {
			...
			s3 = (new StringBuilder()).append(s3).append("\n").toString();
			s3 = (new StringBuilder()).append(s3).append("Please report this bug to the FireHack discord server and @WASTED987 or @29, and we'll try our best to fix it!.\n").toString();
