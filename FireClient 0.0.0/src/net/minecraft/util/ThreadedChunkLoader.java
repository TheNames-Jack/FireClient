...
public class ThreadedChunkLoader implements IChunkLoader, IThreadedFileIO {
	...
	public Chunk loadChunk(World world, int i, int j) throws IOException {
		...
		if(!chunk.isAtLocation(i, j)) {
			System.out.println((new StringBuilder()).append("Chunk file at ").append(i).append(",").append(j).append(" is in the wrong location; relocating. (Expected ").append(i).append(", ").append(j).append(", got ").append(chunk.xPosition).append(", ")
