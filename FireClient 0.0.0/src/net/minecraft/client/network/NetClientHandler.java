...
import net.minecraft.entity.TileEntityBrewingStand;
import net.minecraft.entity.TileEntityDispenser;
import net.minecraft.entity.TileEntityFurnace;
import net.minecraft.entity.TileEntitySign;
...
import net.minecraft.gui.GuiSavingLevelString;
...
import net.minecraft.item.map.MapStorage;
...
public class NetClientHandler extends NetHandler {
	...
	public void handleNamedEntitySpawn(Packet20NamedEntitySpawn packet20namedentityspawn) {
		...
		EntityOtherPlayerMP entityotherplayermp = new EntityOtherPlayerMP(mc.theWorld, packet20namedentityspawn.name, packet20namedentityspawn.uuid);
	...
	public void handleMapChunk(Packet51MapChunk packet51mapchunk) {
		worldClient.invalidateBlockReceiveRegion(packet51mapchunk.xPosition, packet51mapchunk.yPosition, packet51mapchunk.zPosition, (packet51mapchunk.xPosition + packet51mapchunk.xSize) - 1, (packet51mapchunk.yPosition + packet51mapchunk.ySize) - 1,
	...
	public void handleHandshake(Packet2Handshake packet2handshake) {
		...
		if(!flag) {
			...
			addToSendQueue(new Packet1Login(mc.session.username, 22, mc.session.uuid));
		}else {
			try {
				URL url = new URL((new StringBuilder()).append("http://session.minecraft.net/game/joinserver.jsp?user=").append(mc.session.username).append("&sessionId=").append(mc.session.token).append("&serverId=").append(packet2handshake.username)
						.toString());
	...
	public void handlePlayerInfo(Packet201PlayerInfo packet201playerinfo) {
		GuiSavingLevelString guisavinglevelstring = (GuiSavingLevelString) playerInfoMap.get(packet201playerinfo.username);
		if(guisavinglevelstring == null && packet201playerinfo.isConnected) {
			guisavinglevelstring = new GuiSavingLevelString(packet201playerinfo.username);
			playerInfoMap.put(packet201playerinfo.username, guisavinglevelstring);
			playerNames.add(guisavinglevelstring);
		}
		if(guisavinglevelstring != null && !packet201playerinfo.isConnected) {
			playerInfoMap.remove(packet201playerinfo.username);
			playerNames.remove(guisavinglevelstring);
		}
		if(packet201playerinfo.isConnected && guisavinglevelstring != null) {
			guisavinglevelstring.ping = packet201playerinfo.ping;
		}
	}
