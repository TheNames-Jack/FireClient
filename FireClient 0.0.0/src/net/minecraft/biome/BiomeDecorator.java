...
import net.minecraft.item.map.MapGenWaterlily;
import net.minecraft.world.World;
import net.minecraft.world.WorldGenBigMushroom;
import net.minecraft.world.WorldGenCactus;
import net.minecraft.world.WorldGenClay;
import net.minecraft.world.WorldGenDeadBush;
import net.minecraft.world.WorldGenFlowers;
import net.minecraft.world.WorldGenLiquids;
import net.minecraft.world.WorldGenMinable;
import net.minecraft.world.WorldGenPumpkin;
import net.minecraft.world.WorldGenReed;
import net.minecraft.world.WorldGenSand;
import net.minecraft.world.WorldGenTallGrass;
...
public class BiomeDecorator {
	...
	protected void decorate_do() {
		...
		for(int j2 = 0; j2 < flowersPerChunk; j2++) {
			...
			int i11 = decoRNG.nextInt(currentWorld.worldYMax);
			...
			if(decoRNG.nextInt(4) == 0) {
				...
				int j11 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		for(int k2 = 0; k2 < grassPerChunk; k2++) {
			...
			int j15 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		for(int l2 = 0; l2 < deadBushPerChunk; l2++) {
			...
			int l11 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		for(int i3 = 0; i3 < waterlilyPerChunk; i3++) {
			...
			int l15;
			for(l15 = decoRNG.nextInt(currentWorld.worldYMax); l15 > 0 && currentWorld.getBlockId(l7, l15 - 1, i12) == 0; l15--) {
			}
		...
		for(int j3 = 0; j3 < mushroomsPerChunk; j3++) {
			...
			if(decoRNG.nextInt(8) == 0) {
				...
				int j16 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		if(decoRNG.nextInt(4) == 0) {
			...
			int k8 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		if(decoRNG.nextInt(8) == 0) {
			...
			int l8 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		for(int i4 = 0; i4 < reedsPerChunk; i4++) {
			...
			int k16 = decoRNG.nextInt(currentWorld.worldYMax);
			reedGen.generate(currentWorld, decoRNG, i9, k16, j13);
		}

		for(int j4 = 0; j4 < 10; j4++) {
			int j9 = chunk_X + decoRNG.nextInt(16) + 8;
			int k13 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		if(decoRNG.nextInt(32) == 0) {
			...
			int k9 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		for(int l4 = 0; l4 < cactiPerChunk; l4++) {
			...
			int i14 = decoRNG.nextInt(currentWorld.worldYMax);
		...
		if(generateFluid) {
			for(int i5 = 0; i5 < 50; i5++) {
				...
				int j14 = decoRNG.nextInt(decoRNG.nextInt(currentWorld.worldYMax - 8) + 8);
			...
			for(int j5 = 0; j5 < 20; j5++) {
				...
				int k14 = decoRNG.nextInt(decoRNG.nextInt(decoRNG.nextInt(currentWorld.worldYMax - 16) + 8) + 8);
